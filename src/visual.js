...
function draw(data) {
    ...
    function redraw() {
        ...
        if (config.use_img) {
            ...
            // logo
            barEnter.append("rect").attr("fill-opacity", 0).attr("cy", 63)
                .attr('fill', d => 'url(#' + d.name + ')')
                .attr("fill-opacity", 1)
                ...
                .attr("width", 40)
                .attr("height", 30)
                .attr("class", function(){return 'logo-text'})
        ...
        // value 右边 图片
        const imgSrc = './img/pao.GIF';
        var imgs = svg.selectAll("image").data([0]);
        imgs.enter()
        var barImg =  barEnter.append("svg:image").attr('xlink:href', imgSrc).attr('width', 100).attr('height', 100).attr('y', -26)
            .attr("x", d => {
                return xScale(xValue(d)) + 10
            }).attr("class", function () {
                return "barImg"
        ...
        // 右边随机数字
        // var nums = barEnter.append("text").attr('width', 100).attr('height', 48).attr('y', 32)
        //     .attr("x", d => {
        //         return xScale(xValue(d)) + 10
        //     }).attr("class", function () {
        //         return "nums"
        ...
        if (config.use_img) {
            ...
            // logo
            barUpdate.select("rect").attr("stroke", function (d) {
                return getColor(d);
            })
        }

        // 图片跟随条形图
        barUpdate.select(".barImg")
            .attr("x", d => {
                return xScale(xValue(d)) + 10
            })
        // 随机数字更新
        // barUpdate.select(".nums")
        //     .attr("x", d => {
        //         return xScale(xValue(d)) + 110
        //     })
        //     .text(d => { 
        //         let a = Math.ceil(xValue(d) * 3);
        //         return a
